...
// Displays a paginated list of help requests where this user has been assigned
// as the helper (i.e. they are providing assistance to someone else).
// Endpoint: GET /users/{userId}/help-requests/helping
...
// ─── Props ────────────────────────────────────────────────────────────────────
type UserHelperHelpRequestsProps = {
  /** The user id whose helper assignments should be loaded */
  userId: string
}

// ─── Status badge ─────────────────────────────────────────────────────────────
// Shows whether the request the user helped with is still open or completed
...
// ─── Component ────────────────────────────────────────────────────────────────
export function UserHelperHelpRequests({ userId }: UserHelperHelpRequestsProps) {
  ...
  useEffect(() => {
    fetchUserHelperHelpRequests(userId, 1)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [userId])

  // Navigate to a different page in the paginated list
  const goToPage = (page: number) => fetchUserHelperHelpRequests(userId, page)
  ...
  return (
    <>
      ...
      {/* ── Card list ────────────────────────────────────────────────────── */}
      <div className="space-y-3">
        {userHelperHelpRequests.map((req) => {
            ...
            >
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="shrink-0">
                  <StatusBadge isCompleted={req.is_completed} />
                </div>
              </div>
              ...
              {/* Bottom row: requester name + dates + rating */}
              <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-muted-foreground">
                {/* Requester name, always set on a help request */}
                ...
                {/* Rating category the requester assigned after completion */}
                {req.rating && (
        ...
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          ...
          <div className="flex items-center gap-1">
            <Button
              ...
              size="icon"
              className="size-7"
              disabled={pagination.current_page <= 1 || userHelperHelpRequestsLoading}
