...
export function UserRequestedHelpRequests({ userId }: UserRequestedHelpRequestsProps) {
  ...
  // ── Empty state ──────────────────────────────────────────────────────────
  if (userRequestedHelpRequests && userRequestedHelpRequests.length === 0) {
    return (
      <div className="flex flex-col items-center gap-2 py-6 text-center text-muted-foreground">
        <HelpCircle className="size-8 opacity-40" />
        <p className="text-sm">No help requests submitted by this user.</p>
      </div>
    )
  }

  if (!userRequestedHelpRequests) return null
